				...
				<div class="card">
					<form action="" method="get">
																...
																<td>
																	<b>
																		<?php
																			...
																			$maxCredit = $row->max_credit ?? $maxCredit;
						...
						<div class="card-footer" <?php if(isset($_GET['print'])){ $transact = "go"; echo ' style = "display: none"; '; } ?>>
							...
							<?php //if($transact == "go"){ ?>								
								<a href = "addtl_only?86d178a053b97f10a65771b2c1ff9621=<?php echo $_GET['86d178a053b97f10a65771b2c1ff9621'];?>&barcode=&qty=&transact=go&checkout=<?php echo (isset($lunch) && $lunch != '' ? '&lunch=1' : '');?><?php echo (isset($breakfast) && $breakfast != '' ? '&breakfast=1' : '');?>" id = "sub_but" <?php if(!isset($_GET['checkout']) || !isset($_GET['86d178a053b97f10a65771b2c1ff9621'])){ ?> style="display: none;" name = "transact" <?php } ?> type="submit" <?php echo $onclick; ?>  class="btn btn-info btn-block btn-sm">
									<i class="fa fa-dot-circle-o"></i>&nbsp;<span>Save</span>
								</a>
								<a href = "addtl_only?86d178a053b97f10a65771b2c1ff9621=<?php echo $_GET['86d178a053b97f10a65771b2c1ff9621'];?>&barcode=&qty=&transact=go&checkout=&print<?php echo (isset($lunch) && $lunch != '' ? '&lunch=1' : '');?><?php echo (isset($breakfast) && $breakfast != '' ? '&breakfast=1' : '');?>" id = "sub_but_print" <?php if(!isset($_GET['checkout']) || !isset($_GET['86d178a053b97f10a65771b2c1ff9621'])){ ?> style="display: none;" name = "transact" <?php } ?> type="submit" <?php echo $onclick; ?>  class="btn btn-info btn-block btn-sm">
	...
	if( isset($_GET['transact']) && $_GET['transact'] != "" && $transact == 'go'){
		...
		//if($stmt->execute() === TRUE){
			...
			if($stmt->execute() === TRUE){				
				...
				if(isset($_GET['print'])){
					alert("addtl_only?86d178a053b97f10a65771b2c1ff9621=".$_GET['86d178a053b97f10a65771b2c1ff9621']."&print=".$_SESSION['receipt']['orno'], $alert);
				}else{
					alert("addtl_only", $alert);
				}
				$_SESSION['total'] = 0;
		...
		if($stmt->execute() === TRUE){
			...
			if($result->num_rows > 0){
				...
				if($row->stock > 0){
					...
					if($maxCredit < $row2->amt+($amount+$_SESSION['total'])){
