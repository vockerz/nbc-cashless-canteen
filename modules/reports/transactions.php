			...
			<div class="col-lg-12">
				...
				<?php
					...
					if(isset($_GET['date_fr']) && isset($_GET['date_to'])){
						...
						$whereSql = " FROM transactions as a left join members as b ON a.rfid_no = b.rfid_no WHERE a.dttm BETWEEN '" . $date_fr . $shift_fr . "' and '" . $date_to . $shift_to . "' and a.active IN (0,2) " . $where;
					...
					<div <?php if(isset($_GET['print'])){ ?> id="reportg" <?php } ?>>
						...
						<div class="card">			
							<div class="card-header">
								...
								<a id = "backs" class="btn btn-success btn-sm pull-right" target = "_blank" href="reports/transactions?print&date_fr=<?php echo $_GET['date_fr'];?>&date_to=<?php echo $_GET['date_to'];?>&product=<?php echo $_GET['product'];?>&shift=<?php echo $_GET['shift'];?>"  style="margin-right: 5px;"><span class="fa fa-print"></span> Print </a>
							...
							<div class="card-body">
								...
								<?php if(!isset($_GET['print']) && $totalPages > 1){
									...
									$productVal = isset($_GET['product']) ? $_GET['product'] : '';
									$basePageUrl = 'reports/transactions?date_fr=' . urlencode($_GET['date_fr']) . '&date_to=' . urlencode($_GET['date_to']) . '&shift=' . urlencode($shiftVal) . '&product=' . urlencode($productVal) . '&submit=';
