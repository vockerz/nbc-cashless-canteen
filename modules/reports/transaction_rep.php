...
<div class="content mt-3">
	<div class="animated fadeIn">
		<div class="row">
			<div class="col-lg-12">
				<form action="reports/transaction_rep" method="get">
					<div class="card">
				...
				<?php
					...
					if(isset($_GET['date_fr']) && isset($_GET['date_to'])){
						...
						$whereSql = " FROM transactions as a LEFT JOIN members as b on a.rfid_no = b.rfid_no WHERE a.isload = 0 and a.dttm BETWEEN '" . $date_fr . $shift_fr . "' and '" . $date_to . $shift_to . "' and a.product_id NOT IN (1,2) and a.active IN (0,2) ";
					...
					<div <?php if(isset($_GET['print'])){ ?> id="reportg" <?php } ?>>
						...
						<div class="card">			
							<div class="card-header">
								<a id = "backs" class="btn btn-success btn-sm pull-right" target = "_blank" href="reports/transaction_rep?print&date_fr=<?php echo $_GET['date_fr'];?>&date_to=<?php echo $_GET['date_to'];?>&trans_export&shift=<?php echo $_GET['shift'];?>"  style="margin-right: 5px;"><span class="fa fa-print"></span> Print </a>
								<a id = "backs" class="btn btn-warning btn-sm pull-right" target = "_blank" href="export.php?date_fr=<?php echo $_GET['date_fr'];?>&date_to=<?php echo $_GET['date_to'];?>&trans_export&shift=<?php echo $_GET['shift'];?>"  style="margin-right: 5px;"><span class="fa fa-save"></span> Export </a>
							</div>
							<div class="card-body">
								<table class="table">
								...
								<?php if(!isset($_GET['print']) && $totalPages > 1){
									...
									$basePageUrl = 'reports/transaction_rep?date_fr=' . urlencode($_GET['date_fr']) . '&date_to=' . urlencode($_GET['date_to']) . '&shift=' . urlencode($shiftVal) . '&submit=';
