					...
					<div class="card-header">
						...
						<form action="" method="get">
							<div class="form-inline">
								<label>Search: </label>&nbsp;&nbsp; <input autocomplete="off" name = "search" <?php if(isset($_GET['search'])){ echo " value = '" . $_GET['search'] . "' ";} ?> style="min-width: 350px;" type = "text" class="form-control form-control-sm input-sm" placeholder = "Search employee.">
								&nbsp;
								<?php
							...
							<tbody align = "center">
								<?php
									...
									if(isset($_GET['search'])){
										$where .= " and ( fname like '%" . mysqli_real_escape_string($conn, $_GET['search']) . "%' or lname like '%" . mysqli_real_escape_string($conn, $_GET['search']) . "%' or mname like '%" . mysqli_real_escape_string($conn, $_GET['search']) . "%')";
									}
									if($access->level == 0){
									...
									if($prod->num_rows > 0){
										...
										while ($row = $prod->fetch_object()) {
											...
											echo '<tr>';
												...
												echo '<td>' . $row->type . '</td>';
												echo '<td>';
													if($access->emp_edit){ 		echo '<a onclick = "edit('.$row->member_id.',\'engr\')" name = "edit" class = "btn btn-sm btn-success"><span class="fa fa-edit"></span></a>&nbsp;'; }
													echo '<a style = "color: white; font-weight: bold;" class="btn btn-warning btn-sm" onclick = "edit(\''.$row->member_id.'\',\'load\')"><span>&#x20B1</span></a>';
													if($access->emp_delete){ echo	' <a onclick = "return confirm(\'Are you sure?\');" href = "accounts/emp?delete=' . $row->member_id . '" class = "btn btn-danger btn-sm" data-toggle="tooltip" title="Delete User"><span class = "fa fa-trash"><span></a>'; }
...
<?php
	if(isset($_POST['add']) && $access->emp_add == 1){
		if(!empty($_POST['fname']) && !empty($_POST['mname']) && !empty($_POST['lname']) && !empty($_POST['emp_no']) && !empty($_POST['section'])){
			...
			$stmt = $conn->prepare("INSERT INTO members (type, fname, mname, lname, address, department, position, rfid_no, qr_code, emp_no, section) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
			$stmt->bind_param("sssssssssss", $_POST['type'], $_POST['fname'], $_POST['mname'], $_POST['lname'], $_POST['address'], $_POST['department'], $_POST['position'], $_POST['rfid_no'], $_POST['qr_code'], $_POST['emp_no'], $_POST['section']);
	...
	if(isset($_POST['update']) && $access->emp_edit == 1){
		if(!empty($_POST['fname']) && !empty($_POST['mname']) && !empty($_POST['lname']) && !empty($_POST['emp_no']) && !empty($_POST['section'])){
			...
			$_POST['department'] = strtoupper($_POST['department']);
			$_POST['position'] = strtoupper($_POST['position']);
			if(isset($_SESSION['edit_rfid']) && $_SESSION['edit_rfid'] != $_POST['rfid_no']){
				savelogs("RFID Edit", "From: " . (isset($_SESSION['rfid_no']) ? $_SESSION['rfid_no'] : '') . ' to ' . $_POST['rfid_no']);
			}
			...
			$stmt = $conn->prepare("UPDATE members SET balance = ?, type = ?, fname = ?, mname = ?, lname = ?, address = ?, department = ?, position = ?, emp_no = ?, section = ?	where member_id = ?");
			$stmt->bind_param("ssssssssssi", $_POST['balance'], $_POST['type'], $_POST['fname'], $_POST['mname'], $_POST['lname'], $_POST['address'], $_POST['department'], $_POST['position'], $_POST['emp_no'], $_POST['section'], $_SESSION['edit_id']);
			if($stmt->execute() === TRUE){
	...
	if(isset($_GET['delete']) && $access->level >= 50 && $access->emp_delete == 1){
